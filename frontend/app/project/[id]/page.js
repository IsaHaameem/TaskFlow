...
export default function ProjectPage() {
    ...
    if (loading || authLoading)
        return (
            <div className="flex justify-center items-center min-h-screen"><p>Loading project...</p></div>
        );
    if (error)
        return (
            <div className="flex justify-center items-center min-h-screen"><p className="text-red-500">Error: {error}</p></div>
        );
    if (!project)
        return (
            <div className="flex justify-center items-center min-h-screen"><p>Project not found.</p></div>
        );
                        ...
                        <div className="bg-white shadow rounded-lg p-6">
                            <div className="space-y-4 divide-y divide-gray-200">
                                {project.members.map(({ user: memberUser, role }) => (
                                    // --- FIX: Add a check to ensure memberUser exists before rendering ---
                                    memberUser && (
