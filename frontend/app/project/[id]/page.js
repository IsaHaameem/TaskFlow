...
export default function ProjectPage() {
  ...
  useEffect(() => {
    if (!authLoading && !token) {
      router.push('/login');
    } else if (!authLoading && token && projectId) {
      fetchProject();
    }
  ...
  const handleConfirmRemoveMember = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5001/api/projects/${projectId}/members/${memberToRemove.user._id}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleRoleChange = async (memberId, newRole) => {
    try {
      const res = await fetch(
        `http://localhost:5001/api/projects/${projectId}/members/${memberId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
    ...
    <>
                    ...
                    <div className="flex items-center gap-x-4">
                      {isAdmin && project.owner !== memberUser._id ? (
                        <select
                          ...
                          className="text-sm font-medium text-gray-700 bg-gray-200 px-3 py-1 rounded-full border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 appearance-none cursor-pointer"
                        >
                      ...
                      {isAdmin && project.owner !== memberUser._id && (
                        <button
                          onClick={() => handleOpenRemoveModal({ user: memberUser, role })}
                ...
                <form onSubmit={handleInviteMember} className="mt-6 border-t pt-6">
                  ...
                  <div className="flex items-center mt-2 gap-4">
                    <input
                      ...
                      className="flex-grow mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-gray-900 placeholder-gray-500"
                      required
                    />
                    <button
                      type="submit"
        ...
        {memberToRemove && (
          <div>
            <p className="text-sm text-gray-600">
              ...
              <span className="font-bold">{memberToRemove.user.name}</span> from this project?
            </p>
            <div className="mt-6 flex justify-end space-x-3">
              <button
                type="button"
                onClick={() => setIsRemoveModalOpen(false)}
                className="py-2 px-4 border rounded-md text-sm font-medium bg-white hover:bg-gray-50 text-gray-700"
              >
                Cancel
              </button>
              <button
                ...
                className="py-2 px-4 border rounded-md text-sm font-medium text-white bg-red-600 hover:bg-red-700"
              >
                Remove Member
              </button>
            </div>
      ...
      <div className="fixed right-6 bottom-6 z-50">
        <button
          ...
          className="h-12 w-12 rounded-full bg-indigo-600 shadow-lg flex items-center justify-center text-white focus:outline-none focus:ring-2 focus:ring-indigo-400"
        ...
        >
          ...
          >
            <path
              ...
              strokeWidth="2"
              d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.862 9.862 0 01-4-.8L3 20l1.2-3.6A8 8 0 113 12"
            />
          </svg>
        </button>
      </div>
      ...
      <div
        className={`fixed right-6 bottom-6 z-40 transform transition-transform duration-300 ${
          isChatOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
        style={{ width: 380, height: '80vh' }}
      >
          ...
          <div className="flex items-center justify-between px-4 py-2 border-b">
            ...
            >
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.536-10.95a1 1 0 10-1.414-1.414L10 8.586 7.879 6.464a1 1 0 10-1.414 1.414L8.586 10l-2.12 2.121a1 1 0 101.414 1.415L10 11.414l2.121 2.121a1 1 0 001.414-1.415L11.414 10l2.122-2.122z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
