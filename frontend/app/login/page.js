...
export default function LoginPage() {
    ...
    // This code runs once when the page loads to test the API connection.
    useEffect(() => {
        const checkApiHealth = async () => {
            // It's important to use the environment variable for the API URL
            const apiUrl = process.env.NEXT_PUBLIC_API_URL;
            if (!apiUrl) {
                console.error("Health Check Error: NEXT_PUBLIC_API_URL is not defined!");
                return;
            }
            
            const healthCheckUrl = `${apiUrl}/api/health`;
            console.log(`Pinging API health at: ${healthCheckUrl}`);
